...
export async function createObserver({
  ...
  async function mutationHandler(mutationList) {
    for (const mutation of mutationList) {
      ...
      const links = getAllTwitterLinks(mutation.addedNodes);
  ...
  function getAllTwitterLinks(nodes) {
    const elems = [];
    for (const node of nodes) {
      ...
      const links = [...node.querySelectorAll('a')].filter((x) =>
  ...
  function handleTwitterLink(link) {
    ...
    if (link.dataset && link.dataset.hxObserverDisabled) {
      debug.trace('Observer disabled, skip!');
      return;
    }
  ...
  function updateTwitterFollowers(user, linkHref) {
    ...
    for (let elem of elems) {
      if (!user) {
        ...
        elem.classList.toggle('hx-twitter-link-expired', false);
        elem.dataset.hxFollowersNum = 123;
        elem.dataset.hxFollowers = kFormatter(123000);
        elem.title = `@BitcoinWhales\n15.123 followers 2 days ago`;
        continue;
      }
      ...
      if (!user.expired) {
        ...
        elem.classList.toggle('hx-twitter-link', false);
        elem.classList.toggle('hx-twitter-link-expired', true);
      }
    }
